...
void parse_arg(int argc, char* argv[], struct my_args_t *my_args)
{
    ...
    my_args->o_file = "data/vibration_data.raw";
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        else
        {
            ...
            fprintf(stdout, "Usage: %s [-p tty_device] [-o output_raw_file]\n", argv[0]);
            exit(EXIT_FAILURE);
        }
    }
    fprintf(stdout, "Read port: %s\nOutput file: %s\n", my_args->tty_dev, my_args->o_file);
}
...
FILE* set_output_file(char *o_file)
{
    FILE *fp = fopen(o_file, "w");
    return fp;
}
...
int main(int argc, char* argv[])
{
    ...
    fraw = set_output_file(my_args.o_file);
    if (!fraw)
    {
    ...
    // Writing binary data - no CSV header needed
    while(sig == 0)
    {
        ...
        if(state == STATE_FIND_EOF)
        {
            ...
            // Write raw data
            uint32_t data_timestamp = (uint32_t)frame_buff[1]; // in milliseconds
            
            // Write raw 4-byte timestamp
            fwrite(&data_timestamp, sizeof(uint32_t), 1, fraw);
            
            // Write raw data values (4 bytes each)
            for(int i = 2; i < FRAME_TOTAL_INTS - 1; i++)
            {
                fwrite(&frame_buff[i], sizeof(int32_t), 1, fraw);
            }
    ...
    close(tty_fd);
    if (fraw) { fclose(fraw); fraw = NULL; }
    return sig == 0 ? EXIT_SUCCESS : (128 + sig); // Return code for termination by signal
}
